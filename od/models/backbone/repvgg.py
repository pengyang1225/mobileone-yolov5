...
class RepVGG(nn.Module):
    ...
    def __init__(self, num_blocks, width_multiplier=None, override_groups_map=None, deploy=False, use_se=False,pretrained=''):
        ...
        self.sppcsp = SPPCSPC_group(c5_channel, 512)

        self.out_shape = {'C3_size': c3_channel,
                          'C4_size': c4_channel,
                          'C5_size': 512}

        self.out_features =["stage2","stage3","stage4"]
...
import os
def create_RepVGG_A0(deploy=False,pretrained=''):
    model =RepVGG(num_blocks=[2, 4, 14, 1],
                  width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=None, deploy=deploy,pretrained=pretrained)
...
if __name__ == '__main__':
    ...
    model2=repvgg_model_convert(model)
    model.eval()
    model2.eval()
    for i in range(10):
        ...
        start = time.time()
        aaa = model(x)
        print("model_base using time=",time.time() -start)
        start = time.time()
        bbb = model2(x)

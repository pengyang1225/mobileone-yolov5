...
from od.models.modules.common import Conv, Concat, C3,BaseConv,CSPLayer,DWConv,SPPCSPC_group
...
class MobileOneNet(nn.Module):
    def __init__(self, blocks, ks, channels, strides, width_muls, num_classes, deploy=False):
        ...
        self.out_shape = {'C3_size': 128,
                          'C4_size': 256,
                          'C5_size': 512}
        self.out_features =["stage2","stage4","stage5"]
        self.sppcsp = SPPCSPC_group(1024, 512)
...
import os
def make_mobileone_s0(deploy=False,pretrained=''):
    blocks = [
        1,2,8,5,5,1
    ]
    strides = [
    ...
    if os.path.exists(pretrained):
       ...
       # checkpoint = torch.load("mobileone_s0_hello_best.pth.tar")
...
func_dict = {
's0': make_mobileone_s0,

}

def mobileone(**kwargs):
    version = str(kwargs.pop('version'))
    deploy = bool(kwargs.pop('deploy'))
    pretrained = str(kwargs.pop('pretrained'))
    return func_dict[version](deploy,pretrained)


def test_deploy():
    model = make_mobileone_s0(deploy=True)
    checkpoint = torch.load(
        "/media/py/8ee085fc-6bf0-41db-8bee-dbf383701c5c/code/pycharm/flexible-yolov5/weights/mobileone_depoly_model.pt")
    if 'model' in checkpoint:
        checkpoint = checkpoint['model']
    if 'state_dict' in checkpoint:
        checkpoint = checkpoint['state_dict']
    ...
    for i in range(10):
        ...
        with torch.no_grad():
            start = time.time()
            deploy_p = model(x)
            print("model_conver using time=", time.time() - start)


def convert():
    train_model = make_mobileone_s0(deploy=False)
    train_checkpoint = torch.load("/media/py/8ee085fc-6bf0-41db-8bee-dbf383701c5c/code/pycharm/flexible-yolov5/weights/mobileone_s0_hello_best.pth.tar")
    if 'model' in train_checkpoint:
        train_checkpoint = train_checkpoint['model']
    if 'state_dict' in train_checkpoint:
        train_checkpoint = train_checkpoint['state_dict']
    train_ckpt = {}
    for k, v in train_checkpoint.items():
        if k.startswith('module.'):
            ...
            train_ckpt[k] = v
    train_model.load_state_dict(train_ckpt,strict=False)

    model = make_mobileone_s0(deploy=True)
    checkpoint = torch.load("/media/py/8ee085fc-6bf0-41db-8bee-dbf383701c5c/code/pycharm/flexible-yolov5/weights/mobileone_depoly_model.pt")
    if 'model' in checkpoint:
        checkpoint = checkpoint['model']
    if 'state_dict' in checkpoint:
        checkpoint = checkpoint['state_dict']
    ...
    for i in range(10):
        ...
        with torch.no_grad():
            ...
            train_p = train_model(x)
            print("model_base using time=", time.monotonic() - start)
            start = time.monotonic()
            deploy_p = model(x)
            print("model_conver using time=", time.monotonic() - start)
            print(((train_p["stage2"]- deploy_p["stage2"]) ** 2).sum())

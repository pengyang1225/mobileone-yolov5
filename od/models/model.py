...
class Model(nn.Module):
    def __init__(self, model_config):
        ...
        backbone_out['in_features'] = self.backbone.out_features
        neck_type = model_config.neck.pop('type')
        self.fpn = build_neck(neck_type, **backbone_out)
       ...
       # fpn_out['version'] = model_config.backbone.version
        ...
        model_config.head['in_channels'] = fpn_out
        head_type = model_config.head.pop('type')
        self.detection = build_head(head_type, **model_config.head)
        self.stride = self.detection.stride
        self._initialize_biases(head_type)

        initialize_weights(self)

    def _initialize_biases(self, type='YOLOHead', cf=None):
